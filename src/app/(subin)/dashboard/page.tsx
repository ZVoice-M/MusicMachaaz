...
export default async function DashboardPage() {
  ...
  return (
    <>
      <PageHeader
        title="Dashboard"
        description="Today's attendance, dues, trends, and recent activity."
      />
      {/* Today row — 2 cols mobile, 4 desktop */}
      ...
      {/* Finance row — 2 cols mobile, 5 desktop */}
      <section className="mt-3 grid gap-3 grid-cols-2 xl:grid-cols-5">
      ...
      {/* Insights + payments — stack on mobile */}
      <section className="mt-4 grid gap-4 xl:grid-cols-2">
        <Card>
      ...
      <section className="mt-4">
        <Card>
          <div className="mb-3 flex items-center justify-between">
            <h2 className="font-semibold">Pending Due Alerts</h2>
            <Link href="/pending-dues" className="text-sm text-gold">View all</Link>
          </div>
          <div className="flex flex-wrap gap-2">
            {data.ledger.filter((row) => row.pending_amount > 0).slice(0, 8).map((row) => (
              <Badge key={row.id} tone="yellow">
                {row.student_name}: {currency(row.pending_amount, data.settings.currency)}
              </Badge>
            ))}
            {data.ledger.filter((row) => row.pending_amount > 0).length === 0 && (
              <p className="text-sm text-muted">No pending dues.</p>
            )}
          </div>
        </Card>
      </section>
